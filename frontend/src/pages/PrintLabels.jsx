...
export default function PrintLabels() {
  ...
  const mode = searchParams.get('mode') || 'single' // single, sheet, compact

  useEffect(() => {
    async function load() {
      if (ids) {
        // Load specific sowings
        const idList = ids.split(',')
        const results = await Promise.all(
          idList.map(id => fetch(`/api/sowings/${id}`).then(r => r.json()))
        )
        setSowings(results)
      } else {
        // Load all for batch printing
        const res = await fetch('/api/sowings')
        setSowings(await res.json())
      }
      setLoading(false)
    }
    load()
      ...
      <div className="print:hidden">
        ...
        <div className="px-5 py-4 space-y-3">
          <div className="flex gap-2">
            <Link
              to={`/print?${ids ? `ids=${ids}&` : ''}mode=single`}
              className={`flex-1 text-center py-2.5 rounded-xl text-sm font-medium transition-colors ${mode === 'single' ? 'bg-leaf-600 text-white' : 'bg-white text-soil-600 border border-soil-200'}`}
            >
              Single
            </Link>
            <Link
              to={`/print?${ids ? `ids=${ids}&` : ''}mode=sheet`}
              className={`flex-1 text-center py-2.5 rounded-xl text-sm font-medium transition-colors ${mode === 'sheet' ? 'bg-leaf-600 text-white' : 'bg-white text-soil-600 border border-soil-200'}`}
            >
              Sheet
            </Link>
            <Link
              to={`/print?${ids ? `ids=${ids}&` : ''}mode=compact`}
              className={`flex-1 text-center py-2.5 rounded-xl text-sm font-medium transition-colors ${mode === 'compact' ? 'bg-leaf-600 text-white' : 'bg-white text-soil-600 border border-soil-200'}`}
            >
              Compact
            </Link>
          </div>
          ...
          <button
            ...
            className="w-full bg-leaf-600 text-white font-semibold py-3.5 rounded-2xl text-base shadow-md shadow-leaf-600/20 active:scale-[0.98] transition-transform flex items-center justify-center gap-2"
          >
